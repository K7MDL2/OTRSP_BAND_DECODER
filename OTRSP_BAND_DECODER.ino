 ...
 *   which use the OTRSP protocol to send out serial messaages to radios typically intended for SO2R operation. 
 *   One of the message types is the AUX message whcih is a BCD value (per radio) for the purpose of switching 
 ...
 *   relay for example, or select a transverter's enbable line on band change.
 *   
 *   There is also code here to pass on the CW keying and PTT keying, in this example, from a USB line.  
 *   This is not straight forward depending on what CPU model you choose.  DTR and PTT are signaled by DTR and RTS
 *   and on CPUs like the Nano, DTR is used to reset the CPU to bootload software updates from the PC.
 *   On something like a Nano you can remove the cap on the reset line or likely easier to use a USB to UART TTL 
 *   converter connected to one of your CPU serial port that has the DTR and RTS control lines. 
 *   Not all converters do, or have both.
 *   
 *   You could take the PTT signal and 'OR' it with the BCD value in the code here to route PTT to a transverter 
...
/*
  ...
  ret16 = Serial_USB_GetLineControl();   // look for DTR and RTS for N1MM control of CW and PTT. can only happen on USB serial since the external USB UART has only TX and RX lines connected to CPU
    ...
    if (Serial.available() > 0)
    {
        ...
        {                      
                ...
                {     
                    ...
                    AuxNum = atoi(&AuxCmd[4]);  // Convert the ASCII BCD value to a number
                    ...
                    if (AuxCmd[6] == '\r') // if we find \r then we have a valid full command string
                    {
                        ...
                        {                          
                            ...
                            //digitalWrite(Pin0, AuxNum1); // send the value out to the GPIO                   
                        ...
                        {                           
                            AuxNum2 = AuxNum;  // Value to send to Radio 1 BCD output pins
                            Serial.print("AUX2 = ");
                            Serial.println(AuxNum2);  // write out to the Control register which in turn writes to the GPIO ports assigned.                                        
                            //digitalWrite(Pin0, AuxNum2);
                            for (i=0; i< 20; i++)
                                AuxCmd[i] = '\0' ;
                            return 1; 
                        }
